...
A barplot of the counts  of publications in different journals - journals with >2 included papers are shown sorted by descending frequency order (journals colored by discipline).  

```{r plot journal with discipline colors, fig.dim = c(8, 4)}
# note: colors are added manually
...
count(rawdata_incl, Journal) %>%
 ...
 # geom_text(aes(label = scales::comma(n)), hjust = 0, nudge_y = 1) +
  coord_flip() +
  scale_y_continuous(breaks = seq(0, 20, 1)) +
  scale_fill_manual(values = c("#E57E7E", "#E57E7E", "#C3E57E", "#E57E7E", "#E57E7E", "#C3E57E","#E57E7E", "#C3E57E", "#C3E57E")) + 
  theme(legend.position = "none") + 
  labs(x = "", y = "Article count", title = "Where it was published?")
```
...
```{r prep Species_number summary, fig.dim = c(8, 4)}
...
```{r prep Species_number over 100}

#Filter studies and select a few relevant columns
rawdata_incl %>% 
  filter(Species_number != "NA") %>% 
  mutate(Species_number_NUM = as.integer(Species_number)) %>% 
  filter(Species_number_NUM > 100)  %>% select(c("Author", "Year", "Title", "Journal" , "DOI", "Species_number", "Studied_species_type")) -> rawdata_top10size

#dim(rawdata_top10size) #11 articles, 7 columns

#make a table of excluded  studies
t3 <- rawdata_top10size %>%   DT::datatable(rownames = FALSE, width = "100%", options = list(dom = 't', scrollY = '700px', pageLength = 20))
t3
```
...
```{r plot study species one recolor, fig.dim = c(8, 6)}
#Studied_species_type custom colors: "amphibians" = "#009292", "birds" = "#B66DFF","fishes" = "#006DDB", "mammals" = "#FF6DB6", "other" = "#924900", "reptiles" = "#490092", "unclear" = "#DB6D00")# #assign colors to biological groups, https://r-charts.com/color-palettes/ using paletteer_d("colorBlindness::paletteMartin")

#table(rawdata_incl$Study_species, rawdata_incl$Studied_species_type)

 rawdata_incl %>% filter(Species_number == "1") %>% 
   filter(Study_species != "unclear") %>% select(Study_species, Studied_species_type) %>%
   unique() %>% mutate(species_cols = recode(Studied_species_type, "amphibians" = "#009292", "birds" = "#B66DFF","fishes" = "#006DDB", "mammals" = "#FF6DB6", "other" = "#924900", "reptiles" = "#490092", "unclear" = "#DB6D00")) -> species_colors_table
...
rawdata_incl %>% filter(Species_number == "1") %>% 
  ...
  scale_fill_manual(labels = c("fishes", "reptiles", "birds", "mammals"), values = c("#006DDB", "#490092", "#B66DFF", "#FF6DB6")) +
 # geom_text(aes(label=scales::comma(n)), hjust = 0, nudge_y = 1) +
...
Studied_species_type_sep %>% 
  ...
  geom_bar(stat = "identity", position = "dodge") +
  scale_fill_manual(values=rev(c("#FF6DB6", "#B66DFF", "#006DDB", "#490092", "#009292", "#DB6D00", "#924900"))) +
  geom_text(aes(label = as.integer(scales::comma(n))), hjust = 0, nudge_y = 1) +
  ylim(c(0,150)) +
  coord_flip() +
  theme(legend.position = "none") +
...
#number of papers with 1,2,3... outcomes
Outcome_type_sep %>% 
  ...
  geom_histogram() +
  scale_x_log10() +
  labs(x = "Number of outcomes", y = "Article count", title = "Number of outcomes per article?")
...
rawdata_incl %>% 
  ...
  geom_area() +
  theme(legend.position = "bottom", legend.direction = "vertical") + 
  scale_fill_manual(values = c("#E5B17E", "#C3E57E", "#E57E7E"))+ 
...
```{r plot years and image types, fig.dim = c(8, 4)}
#Image_source_type_sep <- separate_rows(rawdata_incl, Image_source_type, sep = ", ") #split rows with multiple values
#Image_source_type_sep$Image_source_type <- as.factor(Image_source_type_sep$Image_source_type)
#table(Image_source_type_sep$Image_source_type, useNA = "always") #1 NA
...
Image_source_type_sep %>% 
  ...
  geom_area() +
  theme(legend.position = "bottom", legend.direction = "vertical") + 
  labs(x = "", y = "Article count", title = "Image source type by year")
```
...
color_bgroup <- recode(myspecies_data$biological_group, "fishes" = "#009292", "mammals" = "#FF6DB6")
...
text(21, 14, "Species", cex=1.1) #col = "#325F8C", 
title("Individual recognition (re-identification)")
add_phylopic_base(Salmo, x = 60, y = 13, ysize = 0.8, xsize = 8, color = "#009292", alpha = 0.6)
add_phylopic_base(Panthera, x = 60, y = 12, ysize = 1, xsize = 12, color = "#FF6DB6", alpha = 0.7)
add_phylopic_base(Pusa,x = 60, y = 11, ysize = 1, xsize = 10, color = "#FF6DB6", alpha = 0.7)
add_phylopic_base(Ailuropoda, x = 60, y = 10, ysize = 1, xsize = 6, color = "#FF6DB6", alpha = 0.7)
add_phylopic_base(Ursus, x = 60, y = 9, ysize = 1, xsize = 11, color = "#FF6DB6", alpha = 0.7)
add_phylopic_base(Eubalaena, x = 60, y = 8, ysize = 1, xsize = 12, color = "#FF6DB6", alpha = 0.7)
add_phylopic_base(Megaptera, x = 60, y = 7, ysize = 1, xsize = 10, color = "#FF6DB6", alpha = 0.7)
add_phylopic_base(Sousa, x = 60, y = 6, ysize = 1, xsize = 10, color = "#FF6DB6", alpha = 0.7)
add_phylopic_base(Cephalorhynchus, x = 60, y = 5, ysize = 1, xsize = 12, color = "#FF6DB6", alpha = 0.7)
add_phylopic_base(Grampus, x = 60, y = 4, ysize = 1, xsize = 12, color = "#FF6DB6", alpha = 0.7)
add_phylopic_base(Gorilla, x = 60, y = 3, ysize = 1, xsize = 6, color = "#FF6DB6", alpha = 0.7)
add_phylopic_base(Pan, x = 60, y = 2, ysize = 0.8, xsize = 6, color = "#FF6DB6", alpha = 0.7)
add_phylopic_base(Macaca, x = 60, y = 1, ysize = 1, xsize = 8, color = "#FF6DB6", alpha = 0.7)
text(84, 14, "Article count", cex=1.1) #col = "#325F8C", 
text(21, 14, "Species", cex=1.1) #col = "#325F8C", 
title("Individual recognition (re-identification)")
dev.off()
...
#### Table S4
Table of top 10 most pructive countries from bibliometric analyses.  
SCP - single-country papers    
MCP - multi-country papers   
```{r data table country prodactivity articles}
t4 <- results_summary$MostProdCountries %>% DT::datatable(rownames = FALSE, width = "100%", options = list(dom = 't', scrollY = '700px', pageLength = 20)) 
t4
```
Note: 
